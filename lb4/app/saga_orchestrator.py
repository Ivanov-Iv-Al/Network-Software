...
class SagaOrchestrator:
    ...
    async def _reserve_seat(self, ticket: Ticket) -> bool:
        try:
            logger.info(f"Reserving seat {ticket.seat_number} for ticket {ticket.id}")
    ...
    async def _process_payment(self, ticket: Ticket) -> bool:
        try:
            logger.info(f"Processing payment for ticket {ticket.id}, amount: {ticket.price}")
            await asyncio.sleep(1)
    ...
    async def _complete_ticket(self, ticket: Ticket) -> None:
        logger.info(f"Completing ticket {ticket.id}")
    ...
    async def _compensate_create_ticket(self, ticket: Ticket) -> None:
        logger.info(f"Compensating ticket creation for {ticket.id}")
    ...
    async def _compensate_reservation(self, ticket: Ticket) -> None:
        logger.info(f"Starting compensation for ticket {ticket.id} reservation")
        
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Compensation attempt {attempt + 1} of {self.max_retries} for ticket {ticket.id}")
    ...
    async def _handle_saga_failure(self, ticket: Ticket) -> None:
        logger.error(f"Critical saga failure for ticket {ticket.id}")
        ticket.status = TicketStatus.FAILED
        ticket.updated_at = datetime.now()
    ...
    def _log_saga_step(self, ticket_id: int, step: str, status: str, error: str = None):
        ...
        self.log_id_counter += 1
        logger.debug(f"Logged saga step: {step} for ticket {ticket_id} with status {status}")
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            "compensation_attempts": sum(t.compensation_attempts for t in self.tickets.values())
        }