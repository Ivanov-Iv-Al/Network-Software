...
async def create_ticket(ticket: TicketCreate, background_tasks: BackgroundTasks):
    """
    Создание билета с использованием Saga паттерна
    Возвращает 202 Accepted, так как процесс выполняется асинхронно
    """
...
async def get_statistics():
    return saga_orchestrator.get_statistics()
...
async def delete_ticket(ticket_id: int):
    ...
    del saga_orchestrator.tickets[ticket_id]
    if ticket_id in saga_orchestrator.saga_logs:
        del saga_orchestrator.saga_logs[ticket_id]
